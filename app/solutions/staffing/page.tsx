          ...
          <div aria-hidden="true" className="rounded-[10px] overflow-hidden font-mono text-[11px]" style={{ background: "var(--color-bg-2)", border: "1px solid rgba(255,255,255,0.12)", boxShadow: "0 24px 60px -12px rgba(0,0,0,0.55)" }}>
            ...
            <div className="p-3.5 flex flex-col gap-2.5">
              {[
                ...
                <div key={member.initials} className="mkp-row flex items-center gap-2.5 px-2.5 py-2 rounded-[6px]" style={{ border: idx === 0 ? "1px solid var(--color-line-t)" : "1px solid var(--color-line)", background: idx === 0 ? "rgba(45,212,191,0.05)" : "rgba(0,0,0,0.18)", display: "grid", gridTemplateColumns: "22px 1fr auto" }}>
                  <div className="w-5 h-5 rounded-full grid place-items-center font-mono text-[8px]" style={{ background: "rgba(129,140,248,0.2)", border: "1px solid rgba(129,140,248,0.3)", color: "var(--color-violet)" }}>
                    {member.initials}
                  </div>
                  <div>
